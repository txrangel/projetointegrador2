...
<script>
    ...
    // Extrair apenas os níveis e as datas formatadas para o gráfico
const seriesData = tanques.map(tanque => {
    return {
        name: tanque.nome,
        data: tanque.dados.map(dado => dado.nivel),  // Apenas os níveis no eixo Y
    };
});

// Extrair as datas correspondentes
const categorias = tanques[0].dados.map(dado => {
    const data = new Date(dado.data);
    const dia = data.getDate().toString().padStart(2, '0');
    const mes = data.toLocaleString('en-US', { month: 'short' });
    return `${dia} ${mes}`;  // Formata a data como 'DD MMM'
});
...
const options = {
    ...
    series: seriesData,  // Apenas os níveis
    legend: {
        show: false
    },
    stroke: {
    ...
    xaxis: {
        categories: categorias,  // Apenas as datas formatadas dinamicamente
        labels: {
            show: true,
            style: {
                fontFamily: "Inter, sans-serif",
    ...
    yaxis: {
        show: true,  // Mostrar os níveis no eixo Y
    },
};
